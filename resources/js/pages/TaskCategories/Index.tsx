...
export default function Index({
    ...
    const deleteTaskCategory = (taskCategory: TaskCategory) => {
        if (taskCategory.tasks_count === 0) {
            if (
                confirm('Are you sure you want to delete this task category?')
            ) {
                router.delete(
                    route('task-categories.destroy', taskCategory.id),
                );
                                ...
                                <TableRow key={taskCategory.id}>
                                    ...
                                    <TableCell className="flex flex-row gap-x-2 text-right">
                                        <Link
                                            ...
                                            href={`/task-categories/${taskCategory.id}/edit`}
                                        >
                                            Edit
                                        </Link>
                                        <Button
